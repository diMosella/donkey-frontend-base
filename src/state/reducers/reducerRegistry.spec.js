...
describe('(Reducers)', () => {
  ...
  describe('ReducerRegistry.register', () => {
    ...
    it('should add a nested named reducer.', () => {
      reducerRegistry.register('/test2x/test25', (state, action) => 'testState25');
      expect(reducerRegistry.read()).to.be.a('object');
      console.log('Spec::register', Object.keys(reducerRegistry.read()));
      expect(Object.entries(reducerRegistry.read())).to.have.length(3);
      expect(reducerRegistry.read()).to.have.property('/test1');
      expect(reducerRegistry.read()).to.have.property('/test2');
      expect(reducerRegistry.read()).to.have.property('/test2x/test25');
      expect(reducerRegistry.read('/test2x/test25')).to.be.a('function');
      expect(reducerRegistry.read('/test2x/test25')({}, 'TEST')).to.eql('testState25');
    });
  });
  ...
  describe('ReducerRegistry.unregister', () => {
    ...
    it('should remove a named reducer.', () => {
      ...
      reducerRegistry.unregister('/test2');
      expect(reducerRegistry.read()).to.be.a('object');
      expect(Object.entries(reducerRegistry.read())).to.have.length(1);
      expect(reducerRegistry.read()).to.not.have.property('/test1');
      expect(reducerRegistry.read()).to.not.have.property('/test2');
    });
  });
  ...
  describe('ReducerRegistry.addEventListener', () => {
    ...
    it('should trigger a register event.', () => {
      ...
      expect(toggledTrigger1).to.eql(false);
      reducerRegistry.unregister('/test4');
      expect(toggledTrigger1).to.eql(false);
      reducerRegistry.unregister('/test3');
      expect(toggledTrigger1).to.eql(false);
    });
